...
struct VectorField{X<:AbstractVector,Q<:Union{Quaternion,Nothing},}
  ...
  function VectorField(x, q)
    v = new{typeof(x),typeof(q)}(x, q)
    checkvfsanity(v)
    return v
  end
end

@inline function checkvfsanity(v::VectorField{X,Q}) where {X,Q}
  # Static checks:
  ...
  # Runtime checks:
  nvars(first(v.x)) == length(v.x) || error("VectorField orbital ray length disagrees with number of variables in TPSA")
  Q == Nothing || getdef(first(v.q)) == getdef(first(v.x)) || error("Quaternion TPSA definition disagrees with orbital ray TPSA definition")
end
